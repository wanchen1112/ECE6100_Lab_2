...
extern uint16_t g_BHR;
extern uint8_t g_PHT[4096];
...
Pipeline * pipe_init(FILE *tr_file_in){
    ...
    if(BPRED_POLICY){
      ...
      g_BHR = 0;
      memset(g_PHT, 2, 4096*sizeof(uint8_t));
    }
...
void debug_message(Pipeline *p)
{
    ...
    {
        if(p->pipe_latch[0][ii].valid == false)
            continue;
...
void pipe_cycle_WB(Pipeline *p){
  ...
  for(ii=0; ii<PIPE_WIDTH; ii++){
      ...
      if(p->pipe_latch[MEM_LATCH][ii].valid){
...
#ifdef PRINT_DEBUG
          printf("-----------------------------\nWB retired: %d\n", p->stat_retired_inst);
          printf("p->fetch_cbr_stall: %d, p->pipe_latch[MEM_LATCH][ii].tr_entry.op_type: %d\n", p->fetch_cbr_stall, p->pipe_latch[MEM_LATCH][ii].tr_entry.op_type);
#endif
          ...
          if((BPRED_POLICY == 1) && (p->pipe_latch[MEM_LATCH][ii].tr_entry.br_dir == false) && (p->pipe_latch[MEM_LATCH][ii].tr_entry.op_type == 3)){
              p->fetch_cbr_stall = false;
              p->pipe_latch[FE_LATCH][0].stall = false;
          ...
          if((BPRED_POLICY == 2) && (p->pipe_latch[MEM_LATCH][ii].mis_prdt == true) && (p->pipe_latch[MEM_LATCH][ii].tr_entry.op_type == 3)){
              p->fetch_cbr_stall = false;
              p->pipe_latch[FE_LATCH][0].stall = false;
              p->pipe_latch[FE_LATCH][0].valid = false;
#ifdef PRINT_DEBUG
...
/**** pass the dependency test, copy data to ID stage, and update valid/stall for current instr. ****/
      ...
      if(p->pipe_latch[FE_LATCH][ii].stall == false)
      {
          ...
          if((p->pipe_latch[FE_LATCH][ii].tr_entry.op_type == 3) && (BPRED_POLICY))
              (uint32_t)p->b_pred->stat_num_branches++;
...
#ifdef PRINT_DEBUG
      printf("ID valid: %d, src1: %d, src2: %d, dst: %d, seq: %d\n", p->pipe_latch[ID_LATCH][ii].valid, p->pipe_latch[ID_LATCH][ii].tr_entry.src1_reg, p->pipe_latch[ID_LATCH][ii].tr_entry.src2_reg, p->pipe_latch[ID_LATCH][ii].tr_entry.dest, p->pipe_latch[ID_LATCH][ii].op_id);
#endif
  } // end of for loop (ii)
...
void pipe_cycle_FE(Pipeline *p){
  ...
  for(ii=0; ii<PIPE_WIDTH; ii++){
    if((p->pipe_latch[FE_LATCH][ii].stall == false) && (p->pipe_latch[FE_LATCH][ii].valid == false) && (p->fetch_cbr_stall == false))// no valid data, read data from traces
    {
...
#ifdef PRINT_DEBUG
      printf("FE valid: %d, src1: %d, src2: %d, dst: %d, seq: %d, op_type: %d\n", p->pipe_latch[FE_LATCH][ii].valid, p->pipe_latch[FE_LATCH][ii].tr_entry.src1_reg, p->pipe_latch[FE_LATCH][ii].tr_entry.src2_reg, p->pipe_latch[MEM_LATCH][ii].tr_entry.dest, p->pipe_latch[FE_LATCH][ii].op_id, p->pipe_latch[FE_LATCH][ii].tr_entry.op_type);
#endif
    }
#ifdef PRINT_DEBUG
    else
        printf("FE stall: %d, seq: %d, op_type: %d\n", p->pipe_latch[FE_LATCH][ii].stall, p->pipe_latch[FE_LATCH][ii].op_id, p->pipe_latch[FE_LATCH][ii].tr_entry.op_type);
#endif
...
void pipe_check_bpred(Pipeline *p, Pipeline_Latch *fetch_op){
    ...
    if(fetch_op->tr_entry.op_type != 3)
        return;

if(BPRED_POLICY == 1)
{
    if((fetch_op->tr_entry.br_dir == false) && (fetch_op->valid == 1)) // not taken, stall the younger instructions
    {
        p->fetch_cbr_stall = true;
        (uint32_t)p->b_pred->stat_num_mispred++;
#ifdef PRINT_DEBUG
        printf("seq: %d is mispredicted (type: %d), stall all the younger instructions!\n", fetch_op->op_id, fetch_op->tr_entry.op_type);
#endif
    }
}
    
else if(BPRED_POLICY == 2)
{
    bool taken = false;
    
    taken = (uint32_t)p->b_pred->GetPrediction(fetch_op->tr_entry.inst_addr);
    
    if((taken != fetch_op->tr_entry.br_dir) && (fetch_op->valid == 1))
    {
        p->fetch_cbr_stall = true;
        (uint32_t)p->b_pred->stat_num_mispred++;
        fetch_op->mis_prdt = true;
    ...
    {
#ifdef PRINT_DEBUG
        printf("--- seq: %d is predicted right! (type: %d)\n", fetch_op->op_id, fetch_op->tr_entry.op_type);
#endif
    }
    
    p->b_pred->UpdatePredictor(fetch_op->tr_entry.inst_addr, fetch_op->tr_entry.br_dir, taken);
}
